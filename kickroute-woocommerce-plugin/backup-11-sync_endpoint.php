...
function kickroute_create_product_attributes( $post_id, $variations ) {
	...
	if( $variations ){
		foreach ( $variations as $single_variation ){
			foreach ( $single_variation['attributes'] as $sing_att_arr ) {
				$attr[ mb_strtolower( $sing_att_arr['name'], 'UTF-8' ) ][] = $sing_att_arr['value'];				
			}
		}
	}
	...
	if ( $attr ) {
		...
		/*	// If taxonomy doesn't exists we create it.
			...
			$keyslug = str_replace(' ', '-', mb_strtolower($key, 'UTF-8'));
			$len = count($real_attr) ;  $attrvalue[$keyslug] = '';
			for($r=0;$r<$len;$r++){
				$attrvalue[$keyslug]  .= $real_attr[$r] . ' | ' ;
				// $real_attr_arr[$r] = str_replace(' ', '-', strtolower($real_attr[$r]));
			} 
			$attrvalue[$keyslug] = substr($attrvalue[$keyslug], 0, -3); 
	...
	if ( $attr ) {
		foreach ( $attr as $attribute => $attr_value ) {
			$attributee = str_replace(' ', '-', $attribute);
			$attrvalue = $attrvalue[$attributee] ;
			$attributee = sanitize_title( $attributee ); 
			$product_attributes_data[ $attributee ] = array(
				'name'         => ucwords( $attribute ),
				'value'      => $attrvalue,
				'is_visible'   => '1',
				'is_variation' => '1',	
				'is_taxonomy'  => '0'
  ...
  $product = wc_get_product( $product_id );
	if( $variation_data ){
		foreach( $variation_data as $single_variation ){
			...
			foreach ( $single_variation['attributes'] as $sing_att_arr ) {
				$valuee = str_replace(' ', '-', mb_strtolower($sing_att_arr['value'], 'UTF-8'));
				$name = str_replace(' ', '-', mb_strtolower($sing_att_arr['name'], 'UTF-8'));
				$namee = sanitize_title($name) ;
				...
				update_post_meta( $variation_id, 'attribute_' . $namee, $sing_att_arr['value']);
			}
